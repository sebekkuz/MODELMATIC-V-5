...
export class Engine {
  ...
  enqueueOrStart(tok: Token) {
    ...
    const elig = this.cfg.able[tok.currentOp] || [];
    const stId = elig[0];
    if (!stId) throw new Error(`Brak stacji dla op: ${tok.currentOp}`);
  ...
  step() {
    ...
    if (ev.kind === 'END_OP') {
      ...
      if (tok.opIdx >= route.length) {
        ...
        const st: StationState | undefined = this.cfg.stations[ev.data.stId];
        if (st) {
          st.doneCount += 1;
        }
      } else {
